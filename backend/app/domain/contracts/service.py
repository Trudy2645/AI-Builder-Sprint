...
class ContractService:
    ...
    async def get_contract_version_approvals(
        ...
        try:
            ...
            final_approval_requested = await self._repository.has_final_approval_request(
                contract_version_id
            )
        except ContractRepositoryUnavailableError as exc:
            self._database_unavailable(exc)
        return self._approval_response(
            context, approvals, final_approval_requested=final_approval_requested
        )
    ...
    async def approve_contract_version(
        ...
        response = self._approval_response(
            mutation.context,
            mutation.approvals,
            final_approval_requested=final_approval_requested,
        )
    ...
    async def request_final_approval(
        ...
        if context.current_version_id != contract_version_id:
            self._raise(
                status.HTTP_409_CONFLICT,
                "VERSION_CONFLICT",
                "Only the current contract version can request final approval.",
            )
        if context.contract_status != "seller_review":
            self._raise(
                status.HTTP_409_CONFLICT,
                "INVALID_STATE_TRANSITION",
                "The contract is not waiting for seller final approval.",
            )
        try:
            created = await self._repository.request_final_approval(
                contract_id=contract_id,
                contract_version_id=contract_version_id,
                actor_user_id=actor.id,
            )
            ...
            raise AppError(
                status_code=status.HTTP_409_CONFLICT,
                code="VERSION_CONFLICT",
                message="The current contract version changed before the request was sent.",
            ) from exc
        except ContractStateConflictError as exc:
            raise AppError(
                status_code=status.HTTP_409_CONFLICT,
                code="INVALID_STATE_TRANSITION",
                message="The contract is not waiting for seller final approval.",
            ) from exc
        except ContractVersionApprovalAccessError as exc:
            raise AppError(
                status_code=status.HTTP_403_FORBIDDEN,
                code="CONTRACT_ACCESS_DENIED",
                message="You do not have access to request final approval for this contract.",
            ) from exc
        except ContractVersionNotFoundError as exc:
            raise AppError(
                status_code=status.HTTP_404_NOT_FOUND,
            ...
            self._database_unavailable(exc)
        return ContractFinalApprovalRequestResponse(
            contract_id=contract_id,
            contract_version_id=contract_version_id,
            requested=True,
    ...
    def _summary(
        ...
        return ContractSummary(
            ...
            final_approval_requested=record.final_approval_requested,
    ...
    def _seller_item(cls, record: ContractRecord) -> SellerContractListItem:
        return SellerContractListItem(
            ...
            buyer_approved=record.buyer_approved,
            seller_approved=record.seller_approved,
            final_approval_requested=record.final_approval_requested,
