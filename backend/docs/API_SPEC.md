> 버전: Special-challenge-aligned MVP v1.3
> 
> 
...
| 역할 선택/가입 | `POST /auth/signup`, `POST /auth/login` |
...
바이어 마이페이지 상태 탭용 목록이다. `finished`는 취소 또는 서비스 기간 종료 계약을 합쳐 반환한다.
...
- `sort=recommended|popular|price_asc`
- `district=해운대구` 복수 가능
- `people=30`
...
공개 상태가 `published`이고 공급 기간이 지나지 않은 공고만 반환한다.
...
응답은 예상 금액, 계산 기준, 사용한 단가/기간, 환율 기준 시각, 신뢰도와 고지를 포함한다. MVP는 명시 단가 기반 계산을 우선하고 과거 계약 데이터는 보정 근거가 충분할 때만 사용한다. AI는 계산이 아니라 설명을 담당한다.

### `GET /public/listings/{listing_id}`

계약명, 셀러, 지역, 평점, 계약 가능 상태, 공급 수량, 기준 단가, 최소 인원, 공급 기간, 핵심 조항 접힌 목록을 반환한다.
...
셀러 조직이 소유한 draft/ready/published/paused/expired/archived 공고를 반환한다.
...
셀러 검증 상태와 필수 정보, 공개용 AI 분석 완료 여부를 확인하고 `published`로 바꾼다. `POST /pause`, `POST /archive`도 같은 상태 전이 규칙을 사용한다.
...
- 인원/기간/예상 가격을 contract terms에 저장
- `action=sign`이면 `signing`, `request_revision`이면 `draft`

응답은 `contract_id`, `version_no`, 다음 행동 endpoint를 반환한다.
...
| 3 | `feat/public-listings` | `feat(listings): add public listing search` | `GET /public/listings` | 비로그인 접근, 추천/인기/가격 정렬, 부산 구·인원·가격·카테고리·기간 필터, cursor pagination을 구현한다. published이며 기간이 유효한 공고만 반환한다. |
...
| 3 | `feat/public-listings` | `feat(pricing): add listing price estimates` | `POST /public/listings/{id}/price-estimates` | 인원·기간·단가 기반 deterministic 계산을 우선한다. 결과 금액, 근거, 환율 metadata, 고지를 `price_estimates`에 저장한다. |
| 3 | `feat/public-listings` | `test(listings): cover public filters and data exposure` | 위 공개 API | 비공개/기간 만료 공고 제외, 가격 경계값, seller 내부 데이터 비노출을 테스트한다. |
| 4 | `feat/seller-listings` | `feat(seller): add seller dashboard and listing queries` | `GET /seller/dashboard`<br>`GET /seller/listings` | 셀러 계약 테이블과 draft/ready/published/paused/expired 공고 목록을 반환한다. seller organization member만 접근한다. |
...
| 6 | `feat/ai-contract-review` | `feat(db): track bounded contract review agent runs` | 없음 | `005_single_contract_review_agent.sql`로 execution mode, Agent 이름, 최대/실제 iteration, 종료 사유와 비민감 실행 metadata를 `ai_analysis_runs`에 추가한다. |
...
| 7 | `feat/contracts` | `feat(contracts): create contracts from listings` | `POST /listings/{id}/contract-requests` | listing current version/clauses, 개인 buyer, seller organization, 선택적 단체명, 인원·기간·예상 가격을 contract version 1로 snapshot한다. |
| 7 | `feat/contracts` | `feat(contracts): add contract detail and cancellation` | `GET /contracts/{id}`<br>`POST /contracts/{id}/cancel` | buyer/seller 당사자만 상세를 볼 수 있다. 취소 시 열린 수정 요청도 cancelled 처리하고 감사 이벤트를 남긴다. |
...
| 8 | `feat/revisions` | `feat(revisions): create draft and sent revision requests` | `POST /contracts/{id}/revision-requests`<br>`GET /revision-requests/{id}` | 조항 선택/직접 입력, 임시저장, 전송을 구현한다. 기준 contract version이 다르면 `VERSION_CONFLICT`를 반환한다. |
...
| 10 | `feat/notifications-mypage` | `feat(mypage): add buyer contract buckets` | `GET /me/contracts` | draft/seller_review/requested/signed/finished 버킷을 계산한다. finished는 취소 또는 서비스 종료 계약을 포함한다. |
