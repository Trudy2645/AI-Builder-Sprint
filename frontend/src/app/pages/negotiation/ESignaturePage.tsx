...
export function ESignaturePage() {
  ...
  if (!contractId || !versionId) return <div className="mx-auto max-w-[820px]"><PageHeader title="전자서명" description="데모 최종안의 전자서명을 진행합니다." /><div className="rounded-xl border border-border bg-card p-6"><h2 className="font-semibold">2026 해운대 단체 객실 공급 계약</h2><p className="mt-2 text-sm text-muted-foreground">양측 승인된 최종안을 확인하고 바이어 서명을 완료하세요.</p><Button className="mt-5" style={{ background: "var(--navy)" }} onClick={() => { sign("buyer"); navigate(`${base}/signing/complete`); }}><PenLine className="mr-1 size-4" />전자서명 완료</Button></div></div>;
  if (!detail) return <PageHeader title="계약을 불러오는 중" description="전자서명에 필요한 계약 정보를 확인하고 있습니다." />;
  const buyer = detail.parties.find((party) => party.role === "buyer"); const seller = detail.parties.find((party) => party.role === "seller");
  return <div className="mx-auto max-w-[820px]"><Button variant="ghost" onClick={() => navigate(`${base}/signing?contractId=${contractId}&versionId=${versionId}`)}><ArrowLeft className="mr-1 size-4" />최종 검토</Button><PageHeader title="전자서명" description="양측 승인 후 모두싸인 서명 요청과 진행 상태를 관리합니다." /><div className="mb-5 rounded-xl border border-border bg-card p-4"><ContractStepper current={5} /></div><div className="mb-6 rounded-xl border border-border bg-card p-5"><h2 className="font-semibold">{detail.current_version.title}</h2><p className="mt-1 text-sm text-muted-foreground">계약 UUID: {contractId} · 버전 v{detail.current_version.version_no}</p><div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">바이어</p><p>{buyer?.name}</p></div><div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">셀러</p><p>{seller?.name}</p></div></div></div>{!request ? <div className="rounded-xl border border-border bg-card p-5"><h2 className="font-semibold">모두싸인 서명 요청 보내기</h2><p className="mt-1 text-sm text-muted-foreground">요청을 생성하면 입력한 두 이메일로 모두싸인 서명 안내 메일이 발송됩니다. 데모 계정 이메일은 실제 받은편지함이 아니므로 실제 확인 가능한 이메일을 입력하세요.</p><div className="mt-4 grid gap-3 md:grid-cols-2"><Input type="email" value={buyerEmail} onChange={(event) => setBuyerEmail(event.target.value)} placeholder="바이어가 실제로 받는 이메일" /><Input type="email" value={sellerEmail} onChange={(event) => setSellerEmail(event.target.value)} placeholder="셀러가 실제로 받는 이메일" /></div><Button className="mt-4" disabled={busy} style={{ background: "var(--navy)" }} onClick={() => void create()}><PenLine className="mr-1 size-4" />모두싸인 서명 요청 보내기</Button></div> : <div className="rounded-xl border border-border bg-card p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">모두싸인 서명 요청</h2><p className="mt-1 text-sm text-muted-foreground">메일 발송 상태: {request.provider_status ?? request.status}</p></div>{request.status === "completed" ? <CheckCircle2 className="size-7" style={{ color: "var(--success)" }} /> : <Clock className="size-7" style={{ color: "var(--warning)" }} />}</div><p className="mt-3 rounded-lg bg-muted p-3 text-sm">서명은 받은 메일의 모두싸인 링크에서 진행합니다. 메일이 보이지 않으면 스팸함을 확인하고, 아래 상태 동기화를 눌러 최신 상태를 확인하세요.</p><Button className="mt-4" disabled={busy} variant="outline" onClick={() => void sync()}>모두싸인 상태 동기화</Button><p className="mt-4 flex gap-1 text-xs text-muted-foreground"><ShieldCheck className="size-3.5" />완료 화면은 서버 계약 상태가 signed일 때만 표시됩니다.</p></div>}</div>;
}
